...
#
# This file is a modified version of https://github.com/idiap/ESLAM/blob/main/src/common.py
...
def get_sample_uv(H0, H1, W0, W1, n, b, depths, colors, sem_feats=None, rgb_feats=None, gt_label=None, device='cuda:0'):
    ...
    depth = torch.gather(depths, 1, indices)  # (b, n)
    color = torch.gather(colors, 1, indices.unsqueeze(-1).expand(-1, -1, 3))  # (b, n, 3)
...
def get_samples(H0, H1, W0, W1, n, H, W, fx, fy, cx, cy, c2ws, depths, colors,
    ...
    """
    b = c2ws.shape[0]   # b: 相当于图像数量
    i, j, sample_depth, sample_color, sampled_sem_feats, sampled_rgb_feats, sampled_gt_label = get_sample_uv(
        H0, H1, W0, W1, n, b, depths, colors, sem_feats, rgb_feats, gt_label, device=device)
    ...
    return (rays_o.reshape(-1, 3), rays_d.reshape(-1, 3), sample_depth.reshape(-1), sample_color.reshape(-1, 3),
            ...
            sampled_gt_label.reshape(-1) if sampled_gt_label is not None else None)
